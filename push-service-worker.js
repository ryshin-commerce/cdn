const OFFLINE_URL = '/off';
const CACHE_NAME = 'off-page';
...
self.addEventListener('fetch', function (event) {
    ...
    //    return cache.match(OFFLINE_URL).then(function (response) {
    //            if (event.request.method === "GET") {
                  
    //                return response || fetch(event.request).then(function (response) {
    //                    try {
    //                        console.log(event.request);
    //                        for (var p of event.request.headers) { console.log(p); };
    //                        console.log("--------------");
    //                        console.log(response);
    //                        for (var x1 of response.headers) { console.log(x1); };
    ...
    e.waitUntil(
        caches.open(CACHE_NAME).then(function (cache) {
            return cache.addAll([
                ...
                '/lib/modulr/modulr.min.css',
                '/css/CommerceDefault/core.min.css',
                '/images/backgrounds/OFFLINE-min.jpg',
                '/site.webmanifest',
                '/favicon.ico'
            ]);
        })
